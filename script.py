...
def send(message, node_src, node_dst):
    ...
    # Send
    print("Node {} ({}) send <{}> to node {} ({})".format(node_src.id, node_src.address, message, node_dst.id, node_dst.address))
...
if __name__ == "__main__":
    ...
    x.start()
    print("Node {} has started with ip {}. This is the root node.".format(nodes[nb_nodes - 1].id, nodes[nb_nodes - 1].address))